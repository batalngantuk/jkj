        ...
        <table style={{ marginBottom: '12px', fontSize: '10px' }}>
          <thead>
            <tr>
              ...
              <th style={{ width: '13%' }}>No. Dokumen</th>
          ...
          <tbody>
            {rec.items.filter(i => i.isKITE).map((item, i) => (
              <tr key={i}>
                ...
                <td style={{ fontFamily: 'monospace', fontSize: '9px' }}>{rec.subkKiteKirimNo}</td>
                <td style={{ textAlign: 'center' }}>{rec.subkKiteKirimTgl || rec.jobTgl}</td>
          ...
          <tbody>
            {hasResult
              ? rec.items.filter(i => (i.qtyKembali ?? 0) > 0).map((item, i) => (
                <tr key={i}>
                  <td style={{ textAlign: 'center' }}>{i + 1}</td>
                  <td style={{ fontFamily: 'monospace', fontSize: '9px' }}>{rec.subkKiteTerimaNo}</td>
                  <td style={{ textAlign: 'center' }}>{rec.subkKiteTerimaTgl || rec.tglSelesaiAktual || '-'}</td>
                  <td style={{ fontFamily: 'monospace', fontSize: '9px' }}>{item.kodeBB}</td>
                  <td>{item.namaBB}</td>
                  <td style={{ textAlign: 'center' }}>{item.satuanBB}</td>
              ...
              : <tr><td colSpan={8} style={{ textAlign: 'center', color: '#888' }}>Belum ada hasil diterima</td></tr>
            }
