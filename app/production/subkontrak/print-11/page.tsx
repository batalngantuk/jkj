  ...
  return (
    <>
      ...
      <div style={{ maxWidth: '210mm', margin: '0 auto', padding: '10mm', background: '#fff' }}>
        ...
        {/* Job SO Info */}
        {(rec.noSO || rec.qtyCMT) && (
          <table style={{ marginBottom: '8px', fontSize: '11px' }}>
            <tbody>
              <tr>
                {rec.noSO && (
                  <>
                    <td style={{ border: 'none', padding: '2px 0', width: '30%' }}><b>No. SO Referensi</b></td>
                    <td style={{ border: 'none', padding: '2px 0', width: '20%' }}>: {rec.noSO}</td>
                  </>
                )}
                {rec.qtyCMT && (
                  <>
                    <td style={{ border: 'none', padding: '2px 0', width: '30%' }}><b>Target Qty CMT</b></td>
                    <td style={{ border: 'none', padding: '2px 0' }}>: {rec.qtyCMT.toLocaleString()} {rec.satuanCMT || ''}</td>
                  </>
                )}
              </tr>
            </tbody>
        ...
        <table style={{ marginBottom: '16px', fontSize: '10px' }}>
          <thead>
            <tr>
              ...
              <th style={{ width: '11%' }}>Qty Dikirim</th>
              <th style={{ width: '12%' }}>No. SO</th>
              <th style={{ width: '11%' }}>Qty Target</th>
          ...
          <tbody>
            {kiteBBItems.length > 0 ? kiteBBItems.map((item, i) => (
              <tr key={i}>
                ...
                <td style={{ textAlign: 'center', fontSize: '9px' }}>{rec.noSO || '—'}</td>
                <td style={{ textAlign: 'right' }}>{rec.qtyCMT ? rec.qtyCMT.toLocaleString() : '—'}</td>
                <td style={{ textAlign: 'center' }}>{rec.satuanCMT || '—'}</td>
              </tr>
