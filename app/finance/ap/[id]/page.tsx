...
import { ArrowLeft, Banknote, FileText, Calendar, Building2, CheckCircle2 } from 'lucide-react'
...
export default function APDetailPage() {
  ...
  const invoice = invoices.find((inv: APInvoice) => inv.id === id)
    ...
    return (
      <AppLayout>
        <div className="p-6 space-y-4">
          <Link href="/finance/ap"><Button variant="ghost" size="sm" className="gap-2"><ArrowLeft className="h-4 w-4" />Kembali ke AP</Button></Link>
          <Card>
  ...
  function savePay() {
    ...
    let newStatus: APInvoice['status'] = invoice.status
      ...
      <div className="p-6 space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          ...
          {invoice.balance > 0 && invoice.status !== 'PAID' && (
            <Button className="gap-2 bg-orange-600 hover:bg-orange-700" onClick={openPay}>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">No. Invoice Vendor</p>
                  <p className="font-medium font-mono">{invoice.vendorInvoiceNumber}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <FileText className="h-4 w-4 text-muted-foreground mt-0.5 shrink-0" />
                <div>
                  ...
                  <Link href={`/purchasing/po/${invoice.poId}`} className="font-medium text-primary hover:underline font-mono">
                    {invoice.poNumber}
                  </Link>
                </div>
              </div>
              ...
              {invoice.bc20Number && (
                <div>
                  <p className="text-xs text-muted-foreground">No. BC 2.0</p>
                  <p className="font-medium font-mono">{invoice.bc20Number}</p>
                </div>
              )}
              {isUSD && (
                <>
                ...
                <TableBody>
                  {invoice.lineItems.map(item => {
                    const unitPriceDisplay = isUSD
                      ? `${invoice.currency} ${item.unitPrice.toLocaleString('id-ID', { minimumFractionDigits: 2 })}`
                      : formatIDR(item.unitPrice)
                    const subtotalIDR = isUSD ? item.amount * (invoice.exchangeRate || 1) : item.amount
                    const taxIDR = item.taxAmount
                      ? (isUSD ? item.taxAmount * (invoice.exchangeRate || 1) : item.taxAmount)
                      : 0
                    return (
                      <TableRow key={item.id}>
                        <TableCell>{item.description}</TableCell>
                        <TableCell className="text-right">{item.quantity.toLocaleString('id-ID')}</TableCell>
                        <TableCell className="text-right">{unitPriceDisplay}</TableCell>
                        <TableCell className="text-right font-medium">{formatIDR(subtotalIDR)}</TableCell>
                        <TableCell className="text-right text-orange-700">{taxIDR > 0 ? formatIDR(taxIDR) : '—'}</TableCell>
                        <TableCell className="text-right font-semibold">{formatIDR(subtotalIDR + taxIDR)}</TableCell>
                      </TableRow>
                    )
                  })}
                  {(() => {
                    const subtotalAll = invoice.lineItems.reduce((s, item) => {
                      const amt = isUSD ? item.amount * (invoice.exchangeRate || 1) : item.amount
                      return s + amt
                    }, 0)
                    const taxAll = invoice.lineItems.reduce((s, item) => {
                      const tax = item.taxAmount ? (isUSD ? item.taxAmount * (invoice.exchangeRate || 1) : item.taxAmount) : 0
                      return s + tax
                    }, 0)
                    return (
                      <>
                        <TableRow className="bg-muted/20">
                          ...
                          <TableCell className="text-right">{formatIDR(subtotalAll)}</TableCell>
                          <TableCell className="text-right text-orange-700">{taxAll > 0 ? formatIDR(taxAll) : '—'}</TableCell>
                          <TableCell />
                        </TableRow>
                        <TableRow className="bg-muted/30 font-semibold">
                          <TableCell colSpan={5} className="text-right">Total Invoice</TableCell>
        ...
        <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
          ...
          <div className="space-y-4 py-2 overflow-y-auto flex-1 pr-1">
            <div className="grid grid-cols-2 gap-3 text-sm bg-muted/30 p-3 rounded-md">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Sisa Hutang</p>
                <p className="font-bold text-orange-600">{formatIDR(invoice.balance)}</p>
              </div>
            </div>
            ...
            {isUSD && (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <Label className="text-xs">Kurs Bayar ({invoice.currency}/IDR)</Label>
                  <Input
                    type="number"
                ...
                {invoice.exchangeRate && payForm.kurs && invoice.originalAmount && (
                  <div className="col-span-2 rounded-md bg-yellow-50 border border-yellow-200 p-2 text-xs space-y-0.5">
                    <p className="text-muted-foreground">Kurs invoice: <strong>{invoice.exchangeRate.toLocaleString('id-ID')}</strong> | Kurs bayar: <strong>{parseFloat(payForm.kurs).toLocaleString('id-ID')}</strong></p>
                    {(() => {
                      const selisih = (invoice.exchangeRate - parseFloat(payForm.kurs)) * invoice.originalAmount
                      return selisih !== 0 ? (
            ...
            <div className="space-y-2">
              <Label>Nominal Dibayar (IDR) <span className="text-red-500">*</span></Label>
              <Input
                placeholder="0"
          ...
          <DialogFooter className="shrink-0">
            ...
            <Button disabled={!payForm.tanggal || !payForm.nominal} onClick={savePay} className="gap-2 bg-orange-600 hover:bg-orange-700">
              <Banknote className="h-4 w-4" />
              Simpan Pembayaran
            </Button>
          </DialogFooter>
